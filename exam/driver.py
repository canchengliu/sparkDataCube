...
def main_func(sc, rdd, url_dict):
	...
	visits_tmp = rdd.map(mapper1).groupByKey().filter(lambda (x, y): len(y) > 50).collect()
	#visits = map((lambda (x,y): (x, set(y))), visits_tmp)
	visits = map((lambda (x,y): (x, minHash(list(y)))), visits_tmp)
	...
	for v1 in visits:
		...
		for v2 in visits[i:]:
			len2 = len(v2[1])
			lenpair = (len1, len2) if len1 < len2 else (len2, len1)
			...
			if int(v1[0]) < int(v2[0]):
				...
				similarity.append((v2[0], v1[0], similar))
	
	similarity = sorted(similarity, key = lambda record : record[2], reverse=True)
